...
@section('title', 'Edit Pesanan | Panel Admin')

@section('content')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Edit Pesanan</h5>
        ...
        <div class="card-body">
            ...
            <form action="{{ route('pesanan.update', $pesanan->id) }}" method="POST">
                ...
                <div class="form-group mb-3">
                    ...
                    <select id="id_barber" name="id_barber" class="form-control @error('id_barber') is-invalid @enderror">
                        ...
                        @foreach($barbers as $barber)
                            <option value="{{ $barber->id }}" {{ old('id_barber', $pesanan->id_barber) == $barber->id ? 'selected' : '' }}>
                                {{ $barber->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="email" id="email" name="email" class="form-control" value="{{ old('email', $pesanan->email) }}" readonly>
                    @error('email')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" id="telepon" name="telepon" class="form-control" value="{{ old('telepon', $pesanan->telepon) }}" readonly>
                    @error('telepon')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea id="alamat_lengkap" name="alamat_lengkap" class="form-control" required>{{ old('alamat_lengkap', $pesanan->alamat_lengkap) }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <input type="date" name="tgl_pesanan" class="form-control @error('tgl_pesanan') is-invalid @enderror" value="{{ old('tgl_pesanan', $pesanan->tgl_pesanan) }}" required>
                    @error('tgl_pesanan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" id="nominal" name="nominal" class="form-control @error('nominal') is-invalid @enderror" value="{{ old('nominal', $pesanan->nominal) }}" required>
                    @error('nominal')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" id="ongkos_kirim" name="ongkos_kirim" class="form-control" value="{{ old('ongkos_kirim', $pesanan->ongkos_kirim ?? 10000) }}" readonly>
                    @error('ongkos_kirim')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="id_transaksi" class="form-control @error('id_transaksi') is-invalid @enderror" value="{{ old('id_transaksi', $pesanan->id_transaksi) }}">
                    @error('id_transaksi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        pelangganSelect.addEventListener('change', function() {
            ...
            fetch(`/pesanan/get-pelanggan-details/${pelangganId}`)
                ...
                .then(data => {
                    ...
                    if (!alamatInput.value) { // Only pre-fill if empty
        ...
        // If there's an existing barber, trigger the change event to load price
        if (barberSelect.value) {
            barberSelect.dispatchEvent(new Event('change'));
        }

        // If there's an existing customer, trigger the change event to load customer details
